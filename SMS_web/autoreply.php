...
<?php
	...
	if( '' !=$content && '' != $msgto ) {	
		...
		$msg2 = explode( ',', $msg2 );
		foreach( $msg2 as $var ) {
			$query = "insert into replymessage(num, content, flag) 
							values ('$var', '$content', '0')";
			mysql_query( $query );
		}
		echo "<script languge=javascript> alert('添加成功！'); 
				location.href='autoreply_ctl.php'</script>;";
	}
?>
    <form  method="post" name="myform"  onsubmit="return checkdata();" action="autoreply.php">
...
?>
          ...
          <td><table  rules="none"  align="center"  cellpadding="1" cellspacing="10" border="0" >
              <tr bgcolor="">
                <td height="23"  align="left" style="font-size:120%;"> 请将自动回复内容限制在70字以内。当前字数：
